const { requirePresence, requireNonNegative, requirePositive } = require('../shared/Invariants');
...
class AccountProgression {
  ...
  constructor({
    ...
    requirePositive(accountLevel, 'accountLevel');
    requireNonNegative(accountXP, 'accountXP');
    requireNonNegative(totalXPEarned, 'totalXPEarned');
    requireNonNegative(skillPoints, 'skillPoints');
    requireNonNegative(prestigeLevel, 'prestigeLevel');
    requireNonNegative(prestigeTokens, 'prestigeTokens');
    /** @type {string} */
   ...
   */
  addXP(xp) {
    if (typeof xp !== 'number' || xp < 0) throw new Error('xp must be a non-negative number');
    this.accountXP += xp;
