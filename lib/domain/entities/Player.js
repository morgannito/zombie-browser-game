...
class Player {
  ...
  updateStats({ kills, deaths, wave, level, playtime, goldEarned }) {
    if (kills < 0) throw new Error('kills must be >= 0');
    if (deaths < 0) throw new Error('deaths must be >= 0');
    if (playtime < 0) throw new Error('playtime must be >= 0');
    if (goldEarned < 0) throw new Error('goldEarned must be >= 0');
