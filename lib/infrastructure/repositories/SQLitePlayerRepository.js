...
/** Explicit column list to avoid leaking sensitive/future columns */
const PLAYER_COLS = 'id, username, total_kills, total_deaths, highest_wave, highest_level, total_playtime, total_gold_earned, last_seen';
...
class SQLitePlayerRepository extends IPlayerRepository {
  ...
  prepareStatements() {
    this.stmts = {
      findById: this.db.prepare(`SELECT ${PLAYER_COLS} FROM players WHERE id = ?`),
      findByUsername: this.db.prepare(`SELECT ${PLAYER_COLS} FROM players WHERE username = ?`),
  ...
  async findById(id) {
    try {
      if (!id) throw new ValidationError('Player ID is required');
  ...
  async create(player) {
    try {
      ...
      this.stmts.create.run(
        player.id, player.username, player.totalKills, player.totalDeaths,
        player.highestWave, player.highestLevel, player.totalPlaytime, player.totalGoldEarned
      );
      return player;
    } catch (error) {
      if (error instanceof ValidationError) throw error;
  ...
  async update(player) {
    try {
      if (!player || !player.id) throw new ValidationError('Player with valid id is required for update');
      const result = this.stmts.update.run(
        player.username, player.totalKills, player.totalDeaths,
        player.highestWave, player.highestLevel, player.totalPlaytime,
        player.totalGoldEarned, Math.floor(player.lastSeen / 1000), player.id
      );
      if (result.changes === 0) throw new NotFoundError('Player', player.id);
      return player;
    } catch (error) {
  ...
  /**
   * Retrieve top N players ordered by wave/level/kills.
  ...
  /**
   * Build and return a stats summary for a player.
   * @param {string} id
  ...
  async getStats(id) {
    try {
      ...
      return this._buildStats(player);
    } catch (error) {
      if (error instanceof ValidationError || error instanceof NotFoundError) throw error;
