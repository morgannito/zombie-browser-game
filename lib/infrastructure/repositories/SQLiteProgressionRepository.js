...
class SQLiteProgressionRepository {
  ...
  /**
   * Atomically find-or-create account progression record.
   * Runs inside a SQLite transaction to prevent duplicate inserts on concurrent XP grants.
   * @param {string} playerId
   * @param {AccountProgression} defaultProgression - used only when creating
   * @returns {AccountProgression}
   */
  findOrCreate(playerId, defaultProgression) {
    const tx = this.db.transaction(() => {
      const row = this.stmts.findByPlayerId.get(playerId);
      if (row) {
        return { progression: AccountProgression.fromDB(row), created: false };
      }
      this.stmts.create.run(
        defaultProgression.playerId,
        defaultProgression.accountLevel,
        defaultProgression.accountXP,
        defaultProgression.totalXPEarned,
        defaultProgression.skillPoints,
        defaultProgression.prestigeLevel,
        defaultProgression.prestigeTokens,
        JSON.stringify(defaultProgression.unlockedSkills)
      );
      return { progression: defaultProgression, created: true };
    });
    return tx();
  }
  ...
  /**
   * Atomically load progression, apply a mutation function, and persist.
   * Prevents TOCTOU races on operations like skill unlock.
   * @param {string} playerId
   * @param {(progression: AccountProgression) => void} mutateFn - throws on business rule violation
   * @returns {AccountProgression} updated progression
   * @throws {NotFoundError} if no progression record found
   */
  updateAtomic(playerId, mutateFn) {
    const tx = this.db.transaction(() => {
      const row = this.stmts.findByPlayerId.get(playerId);
      if (!row) {
        throw new NotFoundError('AccountProgression', playerId);
      }
      const progression = AccountProgression.fromDB(row);
      mutateFn(progression); // domain validation runs here; throws on rule violation
