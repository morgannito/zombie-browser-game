...
class SQLiteUpgradesRepository extends IUpgradesRepository {
  ...
  async findByPlayerId(playerId) {
    try {
      if (!playerId) throw new ValidationError('Player ID is required');
  ...
  async create(upgrades) {
    try {
      if (!upgrades || !upgrades.playerId) throw new ValidationError('Upgrades with valid playerId is required');
  ...
  async update(upgrades) {
    try {
      if (!upgrades || !upgrades.playerId) throw new ValidationError('Upgrades with valid playerId is required for update');
  ...
  _getOrCreateTx(playerId) {
    const tx = this.db.transaction(() => {
      const row = this.stmts.findByPlayerId.get(playerId);
      if (row) return PermanentUpgrades.fromDB(row);
