...
class SQLiteAchievementRepository {
  ...
  prepareStatements() {
    this.stmts = {
      ...
      getAchievementById: this.db.prepare('SELECT * FROM achievements WHERE id = ?'),
      getPlayerAchievements: this.db.prepare(`
        SELECT pa.*, a.name, a.description, a.points, a.tier, a.icon_url
      ...
      unlockAchievement: this.db.prepare(`
        INSERT OR IGNORE INTO player_achievements (player_id, achievement_id, progress, session_id)
        VALUES (?, ?, ?, ?)
      `),
  ...
  async getAllAchievements() {
    try {
      ...
      return rows.map(row => new Achievement({
        ...
        points: row.points,
        tier: row.tier,
        requirementJson: row.requirement_json,
        hidden: row.hidden === 1,
  ...
  async unlockAchievement(playerId, achievementId, sessionId = null) {
    try {
      const result = this.stmts.unlockAchievement.run(playerId, achievementId, 100, sessionId);
