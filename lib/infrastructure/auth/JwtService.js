...
class JwtService {
  constructor(logger) {
    ...
    if (!process.env.JWT_SECRET) {
      this.logger.warn(
        'JWT_SECRET not configured — tokens will not survive server restarts. ' +
          'Set JWT_SECRET in environment for production use.'
      );
    }
  ...
  socketMiddleware() {
    return (socket, next) => {
      // Bench harness bypass: when DISABLE_AUTH_FOR_TESTS=1 (must never be
      // set in prod), accept any socket and stamp a synthetic userId so the
      // downstream handlers have something to key on.
      if (process.env.DISABLE_AUTH_FOR_TESTS === '1') {
        socket.userId = `bench-${socket.id}`;
        socket.username = `bench-${socket.id}`;
        return next();
      }
