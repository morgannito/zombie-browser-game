...
class SkillEffectsApplicator {
  ...
  static applySkillBonuses(player, skillBonuses, _CONFIG) {
    ...
    try {
      ...
      // PERF: skip debug log allocation (Object.keys+filter) in production
      if (logger.isDebugEnabled && logger.isDebugEnabled()) {
        const applied = [];
        for (const k in skillBonuses) {
          if (skillBonuses[k] && skillBonuses[k] !== 0) {
            applied.push(k);
          }
        }
        logger.debug('Applied skill bonuses to player', { playerId: player.id, bonusesApplied: applied });
      }
  ...
  static checkBerserkerMode(player) {
    ...
    if (healthPercent <= player.berserkerThreshold) {
      if (!player.berserkerActive) {
        ...
        logger.debug('Berserker mode activated', {
          playerId: player.id,
      ...
      if (player.berserkerActive) {
        ...
        logger.debug('Berserker mode deactivated', {
          playerId: player.id
        });
      }
      return false;
  ...
  static handleIncomingDamage(player, damage, attacker = null) {
    ...
    // Check dodge
    if (player.dodgeChance && runPRNG.chance(player.dodgeChance)) {
      blocked = true;
      actualDamage = 0;
