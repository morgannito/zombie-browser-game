...
class PerformanceConfig {
  constructor() {
    ...
    // Configs par mode
    this.configs = {
      // Compétitif: 240Hz tick (sim sub-frame) / 120Hz broadcast.
      // Le broadcast est découplé du tick pour ne pas saturer le pipe WS.
      // Cible matériel solide (CPU multi-core moderne).
      insane: {
        tickRate: 1000, // 1kHz tick — sim quasi-continue (1ms/tick)
        maxZombies: 200,
        maxPlayers: 50,
        maxPowerups: 20,
        broadcastRate: 120, // 120Hz broadcast (8.3ms) — limite bande + CPU client
        spawnMultiplier: 1.0,
        gcInterval: 60000,
        enableDeltaCompression: true,
        enableObjectPools: true,
        // 33 ticks @ 1000Hz = 30Hz pathfind — IA 5x plus réactive que high.
        // Charge lissée par staggerOffset (chaque zombie a son propre tick).
        zombiePathfindingRate: 33
      },

      ultra: {
        tickRate: 240, // 240Hz tick (4.17ms)
        maxZombies: 200,
        maxPlayers: 50,
        maxPowerups: 20,
        broadcastRate: 120, // 120Hz broadcast
        spawnMultiplier: 1.0,
        gcInterval: 60000,
        enableDeltaCompression: true,
        enableObjectPools: true,
        zombiePathfindingRate: 8 // 30Hz pathfind (8 ticks @ 240Hz)
      },
      ...
      high: {
        ...
        maxPlayers: 50,
        maxPowerups: 20,
        broadcastRate: 60, // 60Hz — 30Hz causait stutter à 100ms interp delay (seuls 3 snaps/window)
        spawnMultiplier: 1.0,
    ...
    // Hot-reload: react to PERFORMANCE_MODE changes via ConfigHotReload.
    // Deferred require avoids circular dependency at module load time.
    setImmediate(() => {
      try {
        const hotReload = require('./ConfigHotReload');
        hotReload.on('reload', (parsed) => {
          if (parsed.PERFORMANCE_MODE && parsed.PERFORMANCE_MODE !== this.mode) {
            const next = this.configs[parsed.PERFORMANCE_MODE];
            if (next) {
              this.mode = parsed.PERFORMANCE_MODE;
              this.current = next;
            }
          }
        });
      } catch (err) { logger.warn('ConfigHotReload unavailable (test environment)', { err }); }
    });
  }
  ...
  forceGC() {
    if (global.gc && this.mode === 'low-memory') {
      try {
        ...
        logger.warn('Forced GC failed (--expose-gc flag missing)', { err });
      }
    }
  }
  ...
  logConfig() {
    ...
    console.log(
      `[PerformanceConfig] Tick Rate: ${this.current.tickRate} FPS (${this.tickInterval}ms)`
    );
