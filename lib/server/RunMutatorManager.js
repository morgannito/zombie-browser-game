...
return;
    ...
    Object.entries(mutator.effects).forEach(([key, value]) => {
      if (typeof value !== 'number' || !isFinite(value)) {
        console.warn('[MUTATOR] Invalid effect value for key', key, ':', value, '— skipped');
        return;
      }
      if (typeof effects[key] !== 'number') {
        console.warn('[MUTATOR] Unknown effect key', key, 'in mutator', mutator.id, '— skipped');
        return;
      }
      effects[key] *= value;
    });
...
return;
  ...
  clamp(value, min, max) {
    if (!isFinite(value)) {
      console.warn('[MUTATOR] clamp received non-finite value:', value, '— defaulting to min', min);
      return min;
    }
