...
class EntityManager {
  constructor(gameState, config) {
    ...
    this.bulletPool = new ObjectPool(
      () => ({
        ...
        gravity: 0,
        lifetime: null,
        // PHYSICS FIX: Track last update time for deltaTime calculation
        lastUpdateTime: 0,
        createdAt: 0,
        // Plasma trail: scalar fields to avoid per-frame {x,y} allocations
      ...
      bullet => {
        ...
        // PHYSICS FIX: Reset timing fields
        bullet.lastUpdateTime = 0;
        bullet.createdAt = 0;
        // Plasma trail scalar reset
  ...
  createBullet(params) {
    ...
    // PHYSICS FIX: Initialize timing fields for deltaTime calculation
    const now = Date.now();
    bullet.createdAt = params.createdAt || now;
    bullet.lastUpdateTime = now; // Initialize to creation time

    // LAG COMPENSATION: consumed by BulletUpdater on first tick to fast-forward
    // the bullet through (client_interp + RTT) worth of travel via the swept
    // collision pipeline. Must be reset on pool reuse to avoid stale values.
