...
function _normalizeBulletTiming(params) {
  const monotonicNow = performance.now();
  const wallClockNow = Date.now();
  const rawCreatedAt = params.createdAt;
  const rawLifetime = params.lifetime;

  if (typeof rawCreatedAt === 'number' && rawCreatedAt > EPOCH_MS_THRESHOLD) {
    const ageMs = Math.max(0, wallClockNow - rawCreatedAt);
    const createdAt = Math.max(0, monotonicNow - ageMs);
    const lifetime =
      typeof rawLifetime === 'number'
        ? monotonicNow + Math.max(0, rawLifetime - wallClockNow)
        : null;

    return { createdAt, lifetime, monotonicNow };
  }

  const createdAt = typeof rawCreatedAt === 'number' ? rawCreatedAt : monotonicNow;
  const lifetime = typeof rawLifetime === 'number' ? rawLifetime : null;
...
class BulletPool {
  ...
  _assignBulletParams(bullet, params, defaultSize) {
    const timing = _normalizeBulletTiming(params);
    bullet.x = params.x;
    bullet.y = params.y;
    bullet.vx = params.vx;
    bullet.vy = params.vy;
    ...
    bullet.lifetime = timing.lifetime;
    bullet.createdAt = timing.createdAt;
    bullet.lastUpdateTime = timing.createdAt;
