...
delete uo[k];
  ...
  _buildPublicStateForPlayer(playerId, sanitizedPlayers) {
    ...
    const minX = player.x - AOI_HALF_WIDTH;
    const maxX = player.x + AOI_HALF_WIDTH;
    const minY = player.y - AOI_HALF_HEIGHT;
    const maxY = player.y + AOI_HALF_HEIGHT;
    const gs = this.gameState;
    return {
      players: sanitizedPlayers || this._sanitizePlayers(gs.players),
      zombies: _filterEntitiesByRect(gs.zombies, minX, maxX, minY, maxY),
...
snap.angle = quantiseAngle(snap.angle);
  ...
  emitGameState() {
    ...
    if (sockets) {
      ...
      if (!this._bucketCache) {
        this._bucketCache = new Map();
      } else {
        this._bucketCache.clear();
      }
      const bucketCache = sharedState === null ? this._bucketCache : null;
      ...
      sockets.forEach(socket => {
        ...
        let publicState;
        if (sharedState !== null) {
          publicState = sharedState;
        } else {
          const player = this.gameState.players[socketId];
          const bucketKey = this._aoiBucketKeyFor(player);
          if (bucketKey === null) {
            publicState = this._buildPublicStateForPlayer(socketId, sanitizedPlayers);
          } else {
            let cached = bucketCache.get(bucketKey);
            if (!cached) {
              cached = this._buildPublicStateForBucket(bucketKey, sanitizedPlayers);
              bucketCache.set(bucketKey, cached);
            }
            publicState = cached;
          }
        }
