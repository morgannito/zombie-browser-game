 ...
 * @version 1.2.0 — SpatialGrid AOI + per-socket position cache + small-room fallback
 */
...
// AOI position cache: skip grid rebuild for a socket if the player hasn't
// moved more than this many pixels since the last full AOI computation.
const AOI_CACHE_MOVE_THRESHOLD = 50;
...
class NetworkManager {
  ...
  _buildPublicStateForPlayer(playerId, sanitizedPlayers) {
    ...
    // SMALL-ROOM FAST-PATH: when total dynamic entities < threshold, AOI filtering
    // costs more CPU than it saves bandwidth AND causes visible freeze on entities
    // crossing the AOI boundary while the player moves. Skip filter entirely.
    const totalEntities =
      Object.keys(this.gameState.zombies || {}).length +
      Object.keys(this.gameState.bullets || {}).length +
      Object.keys(this.gameState.particles || {}).length;
    if (totalEntities < AOI_SMALL_ROOM_THRESHOLD * 10) { // 50 entities threshold
      return this._buildPublicState();
    }
  ...
  _fieldDiff(cur, prev) {
    if (!prev) {
      ...
      if (typeof patch.x === 'number') patch.x = Math.round(patch.x * 10) / 10;
      if (typeof patch.y === 'number') patch.y = Math.round(patch.y * 10) / 10;
      if (typeof patch.vx === 'number') patch.vx = Math.round(patch.vx * 10) / 10;
      if (typeof patch.vy === 'number') patch.vy = Math.round(patch.vy * 10) / 10;
      if (typeof patch.angle === 'number') patch.angle = quantiseAngle(patch.angle);
      patch._new = true;
      return patch;
    }
    ...
    for (const key in cur) {
      if (STATIC_FIELDS.has(key)) continue;
      if (DYNAMIC_FIELDS.includes(key)) continue; // already handled
      // Skip internal stamps
      if (key === '_new' || key === '_serverX' || key === '_serverY') continue;
      if (cur[key] !== prev[key]) {
        patch[key] = cur[key];
        changed = true;
      }
  ...
  calculateDelta(current, previous, authoritative = null, socketId = null) {
    // PERF: Reuse pre-allocated pool object — no heap allocation per tick.
    ...
    for (const type of entityTypes) {
      ...
      for (const id in entities) {
        ...
        if (Array.isArray(entity)) {
          ...
          if (typeof clonedEntity.x === 'number') clonedEntity.x = Math.round(clonedEntity.x * 10) / 10;
          if (typeof clonedEntity.y === 'number') clonedEntity.y = Math.round(clonedEntity.y * 10) / 10;
          if (typeof clonedEntity.vx === 'number') clonedEntity.vx = Math.round(clonedEntity.vx * 10) / 10;
          if (typeof clonedEntity.vy === 'number') clonedEntity.vy = Math.round(clonedEntity.vy * 10) / 10;
          if (typeof clonedEntity.angle === 'number') clonedEntity.angle = quantiseAngle(clonedEntity.angle);
  ...
  emitGameState() {
    ...
    if (sockets) {
      ...
      sockets.forEach(socket => {
        ...
        const publicState = this._buildPublicStateForPlayer(socketId, sanitizedPlayers);
      ...
      if (isFull) {
        ...
        outerFbU: for (const type in delta.updated) {
          for (const _ in delta.updated[type]) { fallbackHasChanges = true; break outerFbU; }
        }
        if (!fallbackHasChanges) {
          outerFbR: for (const type in delta.removed) {
            if (delta.removed[type].length > 0) { fallbackHasChanges = true; break outerFbR; }
          }
        }
        if (fallbackHasChanges) {
