...
delete uo[k];
  ...
  _buildPublicStateForBucket(bucketKey, sanitizedPlayers) {
    ...
    return {
      ...
      particles: _filterEntitiesByRect(gs.particles, minX, maxX, minY, maxY),
...
snap.angle = quantiseAngle(snap.angle);
  ...
  _shouldSkipSocket(socketId) {
    ...
    // FIX: store in Map instead of dynamic `this[key]` — prevents memory
    // leak (props were never cleared on disconnect).
    if (!this._socketSkipFlags) {
      this._socketSkipFlags = new Map();
    }
    const flipped = !this._socketSkipFlags.get(socketId);
    this._socketSkipFlags.set(socketId, flipped);
    return flipped;
  }
