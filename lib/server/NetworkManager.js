...
// Raised from 150→250ms: 150 was too aggressive for routine intercontinental
// RTT (typical 100-200ms) and caused half-rate updates that froze the world
// visually whenever the local player moved — the client's interpolation
// buffer ran dry between skipped snapshots. Only genuinely poor links
// (>250ms) now pay the throttle.
const PER_SOCKET_THROTTLE_THRESHOLD_MS = 250;
...
const SERVER_INTERNAL_FIELDS = new Set([
  ...
  // Pathfinding / AI caches — previously leaked into deltas as a huge
  // _cachedNearestPlayer object (70 KB/s bandwidth drain).
  '_cachedNearestPlayer', '_cachedNearestPlayerDist', '_cacheTick',
  '_pathCache', '_pathTarget', '_lastPathfind',
  // Misc server-only state accumulated on zombies
...
continue;
    ...
    // FIX: when the entity changed on any field, always force x/y in the patch.
    // Without this, a zombie whose health-only patch went out kept the client
    // interpolation buffer stale — no fresh `_serverTime` paired with position
    // → zombies visually froze while the local player moved. The extra bytes
    // (2 numbers per updated entity per delta) are cheap and guarantee the
    // temporal anchor on every interaction.
  ...
  calculateDelta(current, previous, authoritative = null, socketId = null) {
    ...
    // Meta-données : wave / room / boss always, walls only when the room
    // actually changed. socketId is null in the shared-broadcast path, so we
    // track the last broadcast room globally.
    ...
    if (roomChanged) {
      if (socketId !== null) this._lastRoomIdBySocket.set(socketId, current.currentRoom);
      else this._lastBroadcastRoomId = current.currentRoom;
    }
    delta.meta = {
      wave: current.wave,
...
snap.angle = quantiseAngle(snap.angle);
  ...
  _broadcastThrottleMultiplier() {
    // Graceful-degradation disabled — broadcast at the full tick rate always.
    return 1;
  }
   ...
   * @returns {boolean}
   */
  _shouldSkipSocket(_socketId) {
    // Per-socket broadcast throttle disabled — small game, always send every
    // tick so clients stay perfectly synchronised even on poor links.
    return false;
  }
    ...
    if (sockets) {
      ...
      if (isFull) {
        const fullState = {
          ...
          zombies: sharedState.zombies,
          bullets: sharedState.bullets,
          particles: sharedState.particles,
          poisonTrails: sharedState.poisonTrails,
          explosions: sharedState.explosions,
          powerups: sharedState.powerups,
          loot: sharedState.loot,
          wave: sharedState.wave,
          walls: sharedState.walls,
          currentRoom: sharedState.currentRoom,
          bossSpawned: sharedState.bossSpawned,
          serverTime,
          full: true
        };
        ...
        const delta = this.calculateDelta(sharedState, sharedPrev, this.gameState);
        delta.serverTime = serverTime;
        let hasChanges = false;
        outerUpdated: for (const type in delta.updated) {
          for (const _ in delta.updated[type]) { hasChanges = true; break outerUpdated; }
        }
        if (!hasChanges) {
          outerRemoved: for (const type in delta.removed) {
            if (delta.removed[type].length > 0) { hasChanges = true; break outerRemoved; }
          }
        }
        if (hasChanges) {
 ...
 fallbackHasChanges = true; break outerFbR;
  ...
  _estimatePayloadBytes(payload) {
    // Sample 1 in 60 ticks to avoid GC pressure from JSON.stringify.
    // The histogram still gets ~1 sample/sec at 60Hz — enough for p95.
  ...
  cleanupPlayer(playerId) {
    ...
    // FIX: clear per-socket throttle skip flag (previously leaked as dynamic prop).
    if (this._socketSkipFlags) {
      this._socketSkipFlags.delete(playerId);
    }
  }
