...
class NetworkManager {
  ...
  _fieldDiff(cur, prev) {
    if (!prev) {
      ...
      // Quantise even on first send so client always uses the same decoding path
      if (typeof patch.x === 'number') patch.x = Math.round(patch.x);
      if (typeof patch.y === 'number') patch.y = Math.round(patch.y);
    ...
    for (const type of entityTypes) {
      ...
      for (const id in entities) {
        ...
        if (Array.isArray(entity)) {
          ...
          // Store quantised values so next-tick _fieldDiff compares apples to apples
          if (typeof clonedEntity.x === 'number') clonedEntity.x = Math.round(clonedEntity.x);
          if (typeof clonedEntity.y === 'number') clonedEntity.y = Math.round(clonedEntity.y);
