...
class NetworkManager {
  constructor(io, gameState) {
    ...
    this._lastRoomId = undefined; // track room changes to avoid resending walls
   ...
   */
  calculateDelta(current, previous, authoritative = null) {
    const delta = {
      updated: {},
      removed: {},
      meta: {}
    ...
    const roomChanged = current.currentRoom !== this._lastRoomId;
    if (roomChanged) {
      this._lastRoomId = current.currentRoom;
    }
    delta.meta = {
      wave: current.wave,
    ...
    if (sockets) {
      ...
      sockets.forEach(socket => {
        ...
        if (isFull) {
          ...
          const delta = this.calculateDelta(publicState, prevState, this.gameState);
