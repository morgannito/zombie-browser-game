...
class PerformanceIntegration {
  ...
  /**
   * Increment the tick counter and wrap at MAX_TICK to avoid double-broadcast
   * at the modulo boundary (tickCounter=0 fires for all cadences).
   */
  incrementTick() {
    this.tickCounter = this.tickCounter >= 1000000 ? 1 : this.tickCounter + 1;
  }
  ...
  /**
   * CRITICAL FIX: Reset tick counter for fresh start
   * Prevents stale counter after server restart
   */
  reset() {
    this.tickCounter = 0;
    this.cleanup();
