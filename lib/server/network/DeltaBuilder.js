...
const SERVER_INTERNAL_FIELDS = new Set([
  ...
  'lastAttackTime',
  'spawnTime',
  '_cachedNearestPlayer',
  '_cachedNearestPlayerDist',
  '_cacheTick',
  '_pathCache',
  '_pathTarget',
  '_lastPathfind',
...
class DeltaBuilder {
  ...
  _acquirePatch() {
    if (this._patchPoolIdx < this._patchPool.length) {
      const obj = this._patchPool[this._patchPoolIdx++];
      // Socket.IO's msgpack parser preserves keys with `undefined`, so pooled
      // patch objects must be scrubbed before reuse or later deltas can erase
      // valid client state by re-sending stale keys as `undefined`.
  ...
  _acquireSnap() {
    if (this._snapPoolIdx < this._snapPool.length) {
      const obj = this._snapPool[this._snapPoolIdx++];
      for (const key in obj) {
        delete obj[key];
      }
      return obj;
   ...
   * Reused objects are scrubbed on acquisition to avoid leaking stale keys
   * with `undefined` values into outbound patches.
   */
  _releaseAll() {
    this._patchPoolIdx = 0;
    this._snapPoolIdx = 0;
  ...
  calculateDelta(current, previous, authoritative = null, socketId = null) {
    ...
    const lastRoomId =
      socketId !== null ? this._lastRoomIdBySocket.get(socketId) : this._lastBroadcastRoomId;
  ...
  _estimatePayloadBytes(payload) {
    ...
    try {
      ...
      logger.warn('Failed to compute byte length of payload', { err });
      return 0;
    }
  }
