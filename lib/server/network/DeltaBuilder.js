...
obj[k] = undefined;
   ...
   *
   * New entities (prev=null) receive all non-server-internal fields + `_new:true`.
   * Existing entities receive only changed dynamic/extra fields.
   * Quantisation applied: x/y/vx/vy → 1dp, angle → 0-255 byte.
   *
   * @param {Object} cur
   * @param {Object|null} prev  null when entity is new
   * @returns {Object|null} patch object, or null when nothing changed
   */
  _fieldDiff(cur, prev) {
    if (!prev) {
      const patch = this._acquirePatch();
      for (const key in cur) {
        if (SERVER_INTERNAL_FIELDS.has(key)) continue;
        patch[key] = cur[key];
      }
...
continue;
  ...
  /**
   * Clone state for next-tick delta comparison.
   *
   * Each entity snapshot is a plain object (not pool-borrowed) because
   * snapshots live across ticks — sharing pool objects with `calculateDelta`
   * would corrupt the previous-state baseline when `_releaseAll` wipes the pool.
   * Quantisation is applied so field comparison in `_fieldDiff` is apples-to-apples.
   *
   * @param {Object} state - output of NetworkManager._buildPublicState()
   * @returns {Object} deep-enough clone suitable as `previous` arg to calculateDelta
   */
  cloneState(state) {
    const cloned = {
      wave: state.wave,
    ...
    for (const type of ENTITY_TYPES) {
      ...
      for (const id in entities) {
        ...
        // Plain object — not pool-borrowed — so it survives across calculateDelta calls.
        const snap = Object.create(null);
        for (const key in entity) {
          if (STATIC_FIELDS.has(key)) continue;
          if (SERVER_INTERNAL_FIELDS.has(key)) continue;
          if (key === '_new' || key === '_serverX' || key === '_serverY' || key === '_serverTime') continue;
          snap[key] = entity[key];
        }
        // Track piercedZombies mutations by length (SERVER_INTERNAL_FIELDS skips the array).
        if (entity.piercedZombies && Array.isArray(entity.piercedZombies)) {
          snap._piercedZombiesLen = entity.piercedZombies.length;
        }
        if (typeof snap.x === 'number') snap.x = Math.round(snap.x * 10) / 10;
        if (typeof snap.y === 'number') snap.y = Math.round(snap.y * 10) / 10;
        if (typeof snap.vx === 'number') snap.vx = Math.round(snap.vx * 10) / 10;
        if (typeof snap.vy === 'number') snap.vy = Math.round(snap.vy * 10) / 10;
        if (typeof snap.angle === 'number') snap.angle = quantiseAngle(snap.angle);
