...
class DeltaBuilder {
  ...
  _releaseAll() {
    for (let i = 0; i < this._patchPoolIdx; i++) {
      ...
      for (const k in obj) obj[k] = undefined;
    }
    this._patchPoolIdx = 0;
  ...
  _fieldDiff(cur, prev) {
    if (!prev) {
      ...
      for (const key in cur) patch[key] = cur[key];
    ...
    for (const key in cur) {
      ...
      if (DYNAMIC_FIELDS.has(key)) continue;
    ...
    if (changed && typeof cur.x === 'number' && typeof cur.y === 'number') {
      if (patch.x === undefined) patch.x = Math.round(cur.x * 10) / 10;
      if (patch.y === undefined) patch.y = Math.round(cur.y * 10) / 10;
    }
  ...
  calculateDelta(current, previous, authoritative = null, socketId = null) {
    ...
    for (let ti = 0; ti < ENTITY_TYPES.length; ti++) {
      ...
      for (const id in currentEntities) {
        ...
        if (patch !== null) updatedType[id] = patch;
      }

      for (const id in previousEntities) {
        if (!currentEntities[id]) {
          if (authoritativeEntities && authoritativeEntities[id]) continue;
          removedType.push(id);
        }
      }
    }
  ...
  _estimatePayloadBytes(payload) {
    ...
    if (this._byteSampleCounter % 60 !== 0) return 0;
    try {
