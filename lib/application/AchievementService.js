...
class AchievementService {
  ...
  async checkAndUnlockAchievements(playerId, sessionStats = {}) {
    try {
      const [allAchievements, playerStats, unlocked] = await Promise.all([
        this.achievementRepository.getAllAchievements(),
        this.playerRepository.getStats(playerId),
        this._getUnlockedSet(playerId)
      ]);
...
return [];
  ...
  async _batchPersist(playerId, candidates, sessionStats) {
    ...
    // Fallback: parallel for repos without batch support (e.g. test mocks)
    const settled = await Promise.all(
      candidates.map(a => repo.unlockAchievement(playerId, a.id, sessionStats.sessionId).then(ok => ({ a, ok })))
    );
    return settled.filter(({ ok }) => ok).map(({ a }) => a);
  }
