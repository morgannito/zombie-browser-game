...
class Container {
   ...
   * Efface l'entree de cache existante AVANT de l'ecrire pour eviter
   * une race condition entre delete et set sur un singleton deja resolu.
   * Non thread-safe dans un contexte multi-thread reel (Node.js : pas de
   * parallelisme synchrone, donc safe en pratique).
   * @param {string} name
  ...
  initialize() {
    ...
    this._registerDailyChallenge();
  }

  /**
   * Enregistre database et repositories.
   * @private
   */
  _registerInfrastructure() {
    this.register('database', () => DatabaseManager.getInstance().getDb());
    ...
    this.register('leaderboardRepository', () => new SQLiteLeaderboardRepository(this.resolve('database')));
    this.register('upgradesRepository', () => new SQLiteUpgradesRepository(this.resolve('database')));
    this.register('progressionRepository', () => new SQLiteProgressionRepository(this.resolve('database')));
    this.register('achievementRepository', () => new SQLiteAchievementRepository(this.resolve('database')));
  }

  /**
   * Enregistre les domain services.
   * @private
   */
  _registerServices() {
    this.register(
  ...
  /**
   * Enregistre les use cases (player, session, leaderboard, upgrades).
   * @private
   */
  _registerUseCases() {
    this.register('createPlayerUseCase', () => new CreatePlayerUseCase(this.resolve('playerRepository')));
    this.register('updatePlayerStatsUseCase', () => new UpdatePlayerStatsUseCase(this.resolve('playerRepository')));
    this.register('saveSessionUseCase', () => new SaveSessionUseCase(this.resolve('sessionRepository')));
    this.register('recoverSessionUseCase', () => new RecoverSessionUseCase(this.resolve('sessionRepository')));
    this.register('disconnectSessionUseCase', () => new DisconnectSessionUseCase(this.resolve('sessionRepository')));
    ...
    this.register(
      ...
      () => new GetLeaderboardUseCase(this.resolve('leaderboardRepository'), this.resolve('leaderboardCache'))
    );
  ...
  /**
   * Enregistre le feature daily challenge.
   * @private
   */
  _registerDailyChallenge() {
    this.register('dailyChallengeRepository', () => new SQLiteDailyChallengeRepository(this.resolve('database')));
    this.register(
      'dailyChallengeService',
      () => new DailyChallengeService(this.resolve('dailyChallengeRepository'))
    );
  }
}

// Singleton module-level — un seul conteneur par process
let _instance = null;

/**
 * Retourne l'instance unique du conteneur (singleton).
 * @returns {Container}
 */
...
/**
 * Reinitialise le singleton (reserve aux tests unitaires).
 * Previent les fuites entre suites de tests (singleton leak).
 * @returns {void}
 */
function reset() {
  _instance = null;
...
module.exports = { getInstance, reset };
