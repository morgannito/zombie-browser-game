...
class PlayerManager {
  ...
  generateUpgradeChoices() {
    ...
    while (choices.length < 3 && selectedKeys.size < upgradeKeys.length) {
      ...
      const availableUpgrades = upgradeKeys.filter(
        key => this.levelUpgrades[key].rarity === targetRarity && !selectedKeys.has(key)
      );
  ...
  addXP(player, xpAmount, onLevelUp) {
    // Guard: reject NaN, Infinity, negative values to avoid infinite level-up loop
    if (!Number.isFinite(xpAmount) || xpAmount <= 0) {
      return;
    }
    player.xp = Math.max(0, (player.xp || 0) + xpAmount);

    // Cap iterations as a defense-in-depth against pathological configs
    let safety = 1000;
    while (player.xp >= this.getXPForLevel(player.level) && safety-- > 0) {
