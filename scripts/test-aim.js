...
async function run() {
  ...
  socket.on('gameState', s => {
    ...
    for (const id in s.zombies) zombies[id] = s.zombies[id];
  });
  socket.on('gameStateDelta', d => {
    ...
    for (const id in d.updated?.zombies || {}) {
      ...
      if (!zombies[id]) zombies[id] = patch;
      else Object.assign(zombies[id], patch);
    }
    for (const id of d.removed?.zombies || []) {
      if (zombies[id]) { zombiesKilledTotal++; delete zombies[id]; }
    }
  });
