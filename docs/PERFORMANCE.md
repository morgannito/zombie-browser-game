...
## Perf Budget (`scripts/perf-budget.json`)

Seuils utilisés par le script de validation CI (`scripts/check-perf-budget.js`) après les optimisations PR #27–#31.

| Clé | Valeur | Justification |
|---|---|---|
| `tickAvgMs` | 2 ms | Post-optim (setImmediate + O(1) broadcast), tick moyen mesuré < 1ms ; marge x2 pour CI |
| `tickMaxMs` | 30 ms | Spike acceptable en cold-start ou GC stop-the-world ; en dessous du budget frame 60 Hz (16ms) × 2 |
| `heapMB` | 150 MB | Object pooling réduit les allocs ; 150 MB = headroom raisonnable pour 50 zombies + 100 bullets en pool |
| `bytesPerSecMB` | 15 MB/s | Delta compression (-70-80%) + throttle adaptatif ; base JSON brute ~60 MB/s → divisé par ~4 |
| `gcPauseMs` | 20 ms | Pause GC tolérable sans dégradation perçue (< 1 frame à 60 Hz × 1.2) |
| `p99LatencyMs` | 100 ms | SLA acceptable jeu multijoueur LAN/WiFi ; 100ms = limite haute perçue comme responsive |
